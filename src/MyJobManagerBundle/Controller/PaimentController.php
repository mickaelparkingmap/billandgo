...
use MyJobManagerBundle\Form\PaimentType;
...
class PaimentController extends Controller
{
     ...
     *
     * @Route("/", name="myjobmanager_paiment_index")
     * @Method("GET")
     */
    public function indexAction()
    {
        ...
        $paiments = $em->getRepository('MyJobManagerBundle:Paiment')->findByRefUser($user);
        return $this->render('MyJobManagerBundle:Paiment:index.html.twig', array(
            'paiments' => $paiments,
            'user' => $user
        ));
    }
     ...
     * add a paiment
     *
     * @Route("/add", name="myjobmanager_paiment_add")
     * @param Request $req
        ...
        if ($req->isMethod('POST')) {
            if (($form->handleRequest($req)->isValid())) {
                ...
                if (($paiment->getRefBill()[0]->getRefUser() == $user) && !($paiment->getRefBill()[0]->getType())) {
                    ...
                    $manager->flush();
                    return $this->redirect($this->generateUrl("myjobmanager_paiment_show", array('id' => $paiment->getId())));
                }
            }
        }
        return $this->render('MyJobManagerBundle:Paiment:add.html.twig', array(
     ...
     * add a paiment
     *
     * @Route("/add/{id}", name="myjobmanager_paiment_add_from_bill")
     * @param Request $req
     * @param int $id
     * @return \Symfony\Component\HttpFoundation\RedirectResponse|\Symfony\Component\HttpFoundation\Response
     */
    public function addFromBillAction(Request $req, int $id)
    {
        ...
        return $this->redirect($this->generateUrl("myjobmanager_document_view", array('id' => $document->getId())));
    }

    /**
     * Finds and displays a paiment entity.
     *
     * @Route("/{id}", name="myjobmanager_paiment_show")
     * @Method("GET")
     */
    public function showAction(Paiment $paiment)
    {
        ...
        return $this->render('MyJobManagerBundle:Paiment:show.html.twig', array(
            'paiment' => $paiment,
            'user' => $user,
    ...
    public function deleteAction(Request $request, Paiment $paiment)
    {
        ...
        return $this->redirectToRoute('myjobmanager_paiment_index');
    }
