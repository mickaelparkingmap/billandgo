...
use MyJobManagerBundle\Entity\Numerotation;
use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use MyJobManagerBundle\Entity\User;
use MyJobManagerBundle\Entity\Document;
use MyJobManagerBundle\Entity\Line;
use Symfony\Component\Form\Form;
use MyJobManagerBundle\Form\LineEstimateType;
use MyJobManagerBundle\Form\LineBillType;
use MyJobManagerBundle\Form\DocumentType;
...
class DocumentController extends Controller
{
     ...
     *
     * @Route("/estimates", name="myjobmanager_estimate_index")
     * @Method("GET")
    ...
    public function indexEstimate()
    {
        ...
        $estimates = $manager->getRepository('MyJobManagerBundle:Document')->findAllEstimate($user->getId());

        return $this->render('MyJobManagerBundle:document:index.html.twig', array(
            'list' => $estimates,
            'user' => $user,
            'type' => 'estimate'
        ));
     ...
     *
     * @Route("/bills", name="myjobmanager_bill_index")
     * @Method("GET")
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function indexBill()
    {
        ...
        $manager = $this->getDoctrine()->getManager();
        $bills = $manager->getRepository('MyJobManagerBundle:Document')->findAllBill($user->getId());

        return $this->render('MyJobManagerBundle:document:indexBill.html.twig', array(
            'list' => $bills,
            'user' => $user,
            'type' => 'bill'
        ));
     ...
     *
     * @Route("/documents/{id}", name="myjobmanager_document_view")
     * @Method({"GET", "POST"})
    ...
    public function viewAction(Request $req, int $id)
    {
        ...
        if ($id > 0) {
            $manager = $this->getDoctrine()->getManager();
            $document = $manager->getRepository('MyJobManagerBundle:Document')->find($id);
            if ($document != NULL) {
                ...
                $taxes = $manager->getRepository('MyJobManagerBundle:Tax')->findAll($id);
                $names = $manager->getRepository('MyJobManagerBundle:Line')->lastLinesNames($user);
                $descriptions = $manager->getRepository('MyJobManagerBundle:Line')->lastLinesDescriptions($user);
                return $this->render('MyJobManagerBundle:document:full.html.twig', array(
                    'document' => $document,
                    'taxes' => $taxes,
                    'names' => $names,
        ...
        return $this->redirect($this->generateUrl("myjobmanager_estimate_index"));
    }
     ...
     * @Route("/documents/{id}/edit/status/{status}", name="myjobmanager_document_edit_status")
     * @Method("GET")
    ...
    public function editStatus(int $id, string $status)
    {
        ...
        if (($id > 0) && (in_array($status, $this->status))) {
            ...
            if ($document != NULL) {
                ...
                $manager->flush();
                return $this->redirect($this->generateUrl("myjobmanager_document_view", array('id' => $id)));
            }
        }
    ...
    private function splitLine ($id, $id_line, $split, $user) {
        $manager = $this->getDoctrine()->getManager();
        $document = $manager->getRepository('MyJobManagerBundle:Document')->find($id);
        $line = $manager->getRepository('MyJobManagerBundle:Line')->find($id_line);
    ...
    private function editLine($id_line, $edit, $user) {
        ...
        if (isset($edit['refTax'])) {
            $new_tax = $manager->getRepository('MyJobManagerBundle:Tax')->find($edit['refTax']);
     ...
     * add an estimate
     *
     * @Route("/estimates/add", name="myjobmanager_estimate_add")
     * @Method({"GET", "POST"})
     * @param Request $req
     * @return \Symfony\Component\HttpFoundation\RedirectResponse|\Symfony\Component\HttpFoundation\Response
     */
    public function addEstimateAction(Request $req)
    {
        ...
        $numerotationArray = $manager->getRepository("MyJobManagerBundle:Numerotation")->findByRefUser($user);
        ...
        if ($req->isMethod('POST')) {
            if (($form->handleRequest($req)->isValid())) {
                ...
                $manager->flush();
                return $this->redirect($this->generateUrl("myjobmanager_document_view", array('id' => $estimate->getId())));
            }
        }
        return $this->render('MyJobManagerBundle:document:addEstimate.html.twig', array(
            'form' => $form->createView(),
            'user' => $user
        ));
    }
     ...
     * add a bill
     *
     * @Route("/bills/add", name="myjobmanager_bill_add")
     * @Method({"GET", "POST"})
     * @param Request $req
     * @return \Symfony\Component\HttpFoundation\RedirectResponse|\Symfony\Component\HttpFoundation\Response
     */
    public function addBillAction(Request $req)
    {
        ...
        return $this->render('MyJobManagerBundle:document:addBill.html.twig', array(
            'form' => $form->createView(),
            'user' => $user
        ));
    }
    ...
    /**
     * @Route("/bills/add/estimate/lines", name="myjobmanager_bill_create_from_lines")
     * @param Request $req
     * @return \Symfony\Component\HttpFoundation\RedirectResponse|\Symfony\Component\HttpFoundation\Response
     */
    public function addBillFromEstimateLines (Request $req)
    {
        ...
        $estimate = $manager->getRepository('MyJobManagerBundle:Document')->find($estimate_id);
        if ($estimate == NULL)
            return $this->redirect($this->generateUrl("myjobmanager_bill_index"));
        ...
        foreach ($lines_id as $line_id) {
            $line = $manager->getRepository('MyJobManagerBundle:Line')->find($line_id);
            if ($line->getRefUser() != $user) {
                $ar401 = ['wrong user'];
                return new Response(json_encode($ar401), 401);
            }
     ...
     *
     * @Route("bills/add/estimate/{estimate_id}", name="myjobmanager_bill_create_from_estimate")
    ...
    public function createFromEstimateAction(Request $req, int $estimate_id)
    {
        ...
        if ($req->isMethod('POST')) {
            $manager = $this->getDoctrine()->getManager();
            $estimate = $manager->getRepository('MyJobManagerBundle:Document')->find($estimate_id);
            if ($estimate == NULL) return $this->redirect($this->generateUrl("myjobmanager_bill_index"));
            if ($estimate->getRefUser() != $user)
            {
                $ar401 = ['wrong user'];
                return new Response(json_encode($ar401), 401);
    ...
    /**
     * @Route("bills/add/project/{project_id}", name="myjobmanager_bill_create_from_project")
    ...
    public function createFromProjectAction(Request $req, int $project_id)
    {
        ...
        if ($req->isMethod('POST')) {
            $manager = $this->getDoctrine()->getManager();
            $project = $manager->getRepository('MyJobManagerBundle:Project')->find($project_id);
            if ($project == NULL) return $this->redirect($this->generateUrl("myjobmanager_bill_index"));
    ...
    /**
     * @Route("documents/{doc_id}/send", name="myjobmanager_document_send_email")
     * @Method("GET")
     * @param int $doc_id
    ...
    public function sendEmail (int $doc_id, Request $req)
    {
        ...
        $document = $manager->getRepository('MyJobManagerBundle:Document')->find($doc_id);
        $contact = $manager->getRepository("MyJobManagerBundle:ClientContact")->find($contact_id);
        if ($document == NULL) return $this->redirect($this->generateUrl("myjobmanager_bill_index"));
        if ($contact == NULL) return $this->redirect($this->generateUrl("myjobmanager_document_view", array('id' => $doc_id)));
        ...
        $message = \Swift_Message::newInstance()
            ->setSubject("Facture : ".$document->getNumber()." de ".$user->getCompanyName())
            ->setFrom(array('buliar_m@etna-alternance.net' => "MyJobManager"))
            ->setTo($contact->getEmail())
            ->setBody(
                $this->renderView(
                    "MyJobManagerBundle:document:mailEstimate.html.twig",
                    array(
                        "type" => $type,
                        "document" => $document,
                        "user" => $user,
        ...
        $manager->flush();
        /*return $this->render("MyJobManagerBundle:document:mailEstimate.html.twig",
            array(
                "type" => $type,
                "document" => $document,
                "user" => $user,
        ...
        if ($document->getType())
            return $this->redirectToRoute("myjobmanager_document_edit_status", array("id" => $doc_id, "status" => "estimated"));
        else
            return $this->redirectToRoute("myjobmanager_document_edit_status", array("id" => $doc_id, "status" => "billed"));
        return $this->redirect($this->generateUrl("myjobmanager_document_view", array('id' => $doc_id)));
    }

    /**
     * @Route("documents/{doc_id}/return/{token}/{answer}", name="myjobmanager_document_email_accept")
     * @Method("GET")
     * @param int $doc_id
     * @param int $token
     * @param int $answer
